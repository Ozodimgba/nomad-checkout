 ...
 */
export function createQR(url: string | URL, size = 512, background = 'white', color = 'white'): QRCodeStyling {
    return new QRCodeStyling(createQROptions(url, size, background, color));
}

/** @ignore */
export function createQROptions(url: string | URL, size = 512, background = 'white', color = 'white'): Options {
    return {
