...
const ChatForm: React.FC<ChatFormProps> = ({ prompt, setPrompt, onSubmit }) => {
  const handleSubmit = async (event: FormEvent) => {
    event.preventDefault();
    onSubmit(prompt);
    setPrompt("");
    (event.target as HTMLFormElement).querySelector("textarea")!.rows = 1;
  };
        ...
        <form className="w-full flex" onSubmit={handleSubmit}>
          <textarea
            ...
            onInput={(e) => {
              ...
              e.currentTarget.rows = lineCount > 10 ? 10 : lineCount;
            }}
          />
          <button
