...
export default function MintButton() {
  ...
  const onClick = async () => {
    ...
    try {
          ...
          },
          {
            createMetadataAccountArgsV3: {
              data: {
                ...
                name: "Daily Subcription", // on-chain name
                symbol: "AI", // on-chain symbol
      ...
      setCookie('signature', transactionSignature, { maxAge: 60 * 6 * 24 });
      // Toast notification
...
const transferUSDC = async () => {
  ...
  const splToken = new PublicKey('4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU'); //You must change this value!
  const amount = BigNumber(0.001);
  ...
  return (
    <Button onClick={transferUSDC} isLoading={isLoading} isDisabled={!publicKey}>
    Ok I agree to pay 5 USDC to Receipt
    </Button>
  );
}
